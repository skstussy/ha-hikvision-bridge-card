...
_pushDebugEntry(entry) {
  ...
  setConfig(config) {
    ...
    this._debugSnapshotPulseMap = this._debugSnapshotPulseMap || {};
    this._debugSnapshotLastActivityAt = Number.isFinite(this._debugSnapshotLastActivityAt) ? this._debugSnapshotLastActivityAt : 0;
  ...
  _normalizeCardMode(value = "full") {
    return String(value || "full").trim().toLowerCase() === "mini" ? "mini" : "full";
  }
  ...
  _buildDebugSnapshot(camAttrs = null) {
    const previousSnapshot = this._lastDebugSnapshot && typeof this._lastDebugSnapshot === "object" ? { ...this._lastDebugSnapshot } : null;
    ...
    this._updateDebugSnapshotPulseMap(previousSnapshot, snapshot);
    this._lastDebugSnapshot = snapshot;
    return snapshot;
  }

  _updateDebugSnapshotPulseMap(previousSnapshot = null, snapshot = null) {
    if (!snapshot || typeof snapshot !== "object") return;
    const prev = previousSnapshot && typeof previousSnapshot === "object" ? previousSnapshot : null;
    const nextMap = { ...(this._debugSnapshotPulseMap || {}) };
    const now = Date.now();
    const keys = [
      "online",
      "ptz_supported",
      "webrtc_overlay_active",
      "speaker_enabled",
      "talk_active",
      "stream_mode",
      "speed",
    ...
    keys.forEach((key) => {
      if (!prev) {
        if (nextMap[key] && now - Number(nextMap[key] || 0) > 2400) delete nextMap[key];
        return;
      }
      ...
      if (previous !== current) {
        nextMap[key] = now;
      } else if (nextMap[key] && now - Number(nextMap[key] || 0) > 2400) {
        delete nextMap[key];
      }
    });
    this._debugSnapshotPulseMap = nextMap;
  }

  _getDebugSnapshotPillMeta(snapshot = {}, key = "", fallbackTone = "neutral") {
    const changedAt = Number(this._debugSnapshotPulseMap?.[key] || 0);
    const changedRecently = changedAt > 0 && (Date.now() - changedAt) <= 2400;
    let tone = fallbackTone;
    const value = snapshot?.[key];
    if (key === "online") tone = value ? "good" : "warn";
    else if (key === "ptz_supported") tone = value ? "good" : "neutral";
    else if (key === "webrtc_overlay_active") tone = value ? "primary" : "neutral";
    else if (key === "speaker_enabled") tone = value ? "primary" : "neutral";
    else if (key === "talk_active") tone = value ? "good" : "neutral";
    else if (key === "playback_active") tone = value ? "primary" : "neutral";
    return {
      tone,
      className: `hik-pill ${tone} hik-debug-snapshot-pill${changedRecently ? " is-live" : ""}`,
    };
  }

  _buildDebugSnapshotActivityMeta() {
    const latestEntry = Array.isArray(this._debugEntries) && this._debugEntries.length ? this._debugEntries[this._debugEntries.length - 1] : null;
    const latestTime = latestEntry?.time ? Date.parse(latestEntry.time) : 0;
    const now = Date.now();
    const age = latestTime ? Math.max(0, now - latestTime) : Number.POSITIVE_INFINITY;
    const live = Number.isFinite(age) && age <= 4000;
    if (live) this._debugSnapshotLastActivityAt = latestTime;
    const activeAge = this._debugSnapshotLastActivityAt ? now - this._debugSnapshotLastActivityAt : Number.POSITIVE_INFINITY;
    return {
      label: live ? "Live" : activeAge <= 30000 ? "Recent" : "Idle",
      className: `hik-pill ${live ? "good" : activeAge <= 30000 ? "primary" : "neutral"} hik-debug-snapshot-pill${live ? " is-live" : ""}`,
    };
  }
  ...
  _sanitizeDebugObject(value, seen = null) {
    ...
    if (typeof Node !== "undefined" && value instanceof Node) {
      const tag = String(value?.nodeName || value?.tagName || value?.localName || "node").toLowerCase();
      return `[Node:${tag}]`;
    }
    if (typeof Event !== "undefined" && value instanceof Event) {
      return {
        type: value.type || "",
        target: this._sanitizeDebugObject(value.target, visited),
        currentTarget: this._sanitizeDebugObject(value.currentTarget, visited),
      };
    }
    if (Array.isArray(value)) {
      if (visited.has(value)) return "[Circular]";
      visited.add(value);
      return value.map((item) => this._sanitizeDebugObject(item, visited));
    }
    if (typeof value === "object") {
      ...
      Object.entries(value).forEach(([key, raw]) => {
        ...
        if (["password", "authorization", "authsig", "token", "access_token", "username"].includes(lower)) {
          ...
          next[key] = this._sanitizeDebugObject(raw, visited);
        }
      });
      return next;
    }
...
_setDebugOverlayRect(nextRect = {}, { persist = true, rerender = true } = {}) {
  const next = this._normalizeDebugOverlayRect({ ...(this._debugOverlayRect || {}), ...(nextRect || {}) });
  this._debugOverlayRect = next;
  if (persist) this._saveDebugOverlayRect();
  this._applyDebugOverlayRectToElement(null, next);
  if (rerender) this._queueRender();
}
...
_getDebugOverlayElements() {
  return {
    overlay: this.querySelector('.hik-debug-terminal-window'),
    handle: this.querySelector('.hik-debug-terminal-head'),
    resizeHandle: this.querySelector('.hik-debug-resize-handle'),
  };
}

_syncDebugOverlayElementRect(rect = null) {
  const { overlay } = this._getDebugOverlayElements();
  if (!overlay) return;
  this._applyDebugOverlayRectToElement(overlay, rect);
}
...
_cleanupDebugOverlayPointerState(pointerId = null, { flush = true, release = true } = {}) {
  const { overlay, handle, resizeHandle } = this._getDebugOverlayElements();
  const matchesPointer = (state) => state && (pointerId == null || state.pointerId === pointerId);
  const dragState = matchesPointer(this._debugOverlayDrag) ? this._debugOverlayDrag : null;
  const resizeState = matchesPointer(this._debugOverlayResize) ? this._debugOverlayResize : null;
  const hadInteraction = !!(dragState || resizeState);

  if (dragState) {
    this._debugOverlayDrag = null;
    if (release && handle && dragState.pointerId != null) {
      try { handle.releasePointerCapture(dragState.pointerId); } catch (err) {}
    }
  }
  if (resizeState) {
    this._debugOverlayResize = null;
    if (release && resizeHandle && resizeState.pointerId != null) {
      try { resizeHandle.releasePointerCapture(resizeState.pointerId); } catch (err) {}
    }
  }
  ...
  if (flush && hadInteraction) this._flushDeferredDebugOverlayRender();
}

_updateDebugOverlayDrag(event) {
  ...
  if (start.active !== true) {
    if (Math.hypot(deltaX, deltaY) < 4) return true;
    start.active = true;
...
_updateDebugOverlayResize(event) {
...
_handleDebugOverlayWindowPointerMove(event) {
  if (this._debugOverlayDrag && this._debugOverlayDrag.pointerId === event.pointerId) {
    this._updateDebugOverlayDrag(event);
    return;
  }
  if (this._debugOverlayResize && this._debugOverlayResize.pointerId === event.pointerId) {
    this._updateDebugOverlayResize(event);
  }
}

_handleDebugOverlayWindowPointerEnd(event) {
  if (this._debugOverlayDrag && this._debugOverlayDrag.pointerId === event.pointerId) {
    this._commitDebugOverlayInteraction(event.pointerId);
    return;
  }
  if (this._debugOverlayResize && this._debugOverlayResize.pointerId === event.pointerId) {
    this._commitDebugOverlayInteraction(event.pointerId);
...
_ensureDebugOverlayWindowHandlers() {
  if (this._boundDebugOverlayWindowHandlers) return;
  this._boundDebugOverlayWindowHandlers = {
    pointermove: (event) => this._handleDebugOverlayWindowPointerMove(event),
    pointerup: (event) => this._handleDebugOverlayWindowPointerEnd(event),
    pointercancel: (event) => this._handleDebugOverlayWindowPointerEnd(event),
  ...
  window.addEventListener('blur', this._boundDebugOverlayWindowHandlers.blur, true);
}

_commitDebugOverlayInteraction(pointerId = null) {
  if (this._debugOverlayInteractionCommitting === true) return;
  const dragState = this._debugOverlayDrag && (pointerId == null || this._debugOverlayDrag.pointerId === pointerId) ? this._debugOverlayDrag : null;
  const resizeState = this._debugOverlayResize && (pointerId == null || this._debugOverlayResize.pointerId === pointerId) ? this._debugOverlayResize : null;
  const state = resizeState || dragState;
  if (!state) return;

  this._debugOverlayInteractionCommitting = true;
  try {
    const next = this._clampDebugOverlayRect(this._debugOverlayRect || state.rect || null);
    this._debugOverlayRect = next;
    this._saveDebugOverlayRect();
    this._cleanupDebugOverlayPointerState(pointerId, { flush: false, release: true });
    this._syncDebugOverlayElementRect(next);
    this._flushDeferredDebugOverlayRender();
  } finally {
    this._debugOverlayInteractionCommitting = false;
  }
}
...
_bindDebugOverlayInteractions() {
  ...
  this._ensureDebugOverlayWindowHandlers();

  if (handle.__hikDebugOverlayBound !== true) {
    handle.__hikDebugOverlayBound = true;
    handle.addEventListener('pointerdown', (event) => {
      ...
      this._cleanupDebugOverlayPointerState(null, { flush: false, release: true });
      const current = this.querySelector('.hik-debug-terminal-window');
      if (!current) return;
      const startRect = this._clampDebugOverlayRect(this._debugOverlayRect);
      this._debugOverlayRect = startRect;
      this._debugOverlayDrag = {
      ...
      current.classList.remove('is-resizing');
      current.classList.remove('is-moving');
      this._applyDebugOverlayRectToElement(current, startRect);
      try { handle.setPointerCapture(event.pointerId); } catch (err) {}
    });
    handle.addEventListener('lostpointercapture', (event) => this._commitDebugOverlayInteraction(event?.pointerId ?? null));
  }

  if (resizeHandle.__hikDebugOverlayBound !== true) {
    resizeHandle.__hikDebugOverlayBound = true;
    resizeHandle.addEventListener('pointerdown', (event) => {
      ...
      current.classList.remove('is-moving');
      current.classList.add('is-resizing');
      this._applyDebugOverlayRectToElement(current, startRect);
      try { resizeHandle.setPointerCapture(event.pointerId); } catch (err) {}
    });
    resizeHandle.addEventListener('lostpointercapture', (event) => this._commitDebugOverlayInteraction(event?.pointerId ?? null));
...
_toggleDebugFilter(kind, value) {
  ...
  _renderDebugSnapshot(camAttrs = {}) {
    ...
    const onlineMeta = this._getDebugSnapshotPillMeta(snapshot, "online", "neutral");
    const ptzMeta = this._getDebugSnapshotPillMeta(snapshot, "ptz_supported", "neutral");
    const overlayMeta = this._getDebugSnapshotPillMeta(snapshot, "webrtc_overlay_active", "neutral");
    const speedMeta = this._getDebugSnapshotPillMeta(snapshot, "speed", "neutral");
    const ptzDurationMeta = this._getDebugSnapshotPillMeta(snapshot, "ptz_duration", "neutral");
    const lensDurationMeta = this._getDebugSnapshotPillMeta(snapshot, "lens_duration", "neutral");
    const streamMeta = this._getDebugSnapshotPillMeta(snapshot, "stream_mode", "neutral");
    const speakerMeta = this._getDebugSnapshotPillMeta(snapshot, "speaker_enabled", "neutral");
    const talkMeta = this._getDebugSnapshotPillMeta(snapshot, "talk_active", "neutral");
    const playbackMeta = this._getDebugSnapshotPillMeta(snapshot, "playback_active", "neutral");
    const activityMeta = this._buildDebugSnapshotActivityMeta();
    return `
      <div class="hik-debug-snapshot-grid">
        <span class="hik-pill neutral hik-debug-snapshot-pill"><ha-icon icon="mdi:cctv"></ha-icon>CH ${this.escapeHtml(snapshot.channel || "-")}</span>
        <span class="${onlineMeta.className}"><ha-icon icon="mdi:lan-connect"></ha-icon>${snapshot.online ? "Online" : "Offline"}</span>
        <span class="${ptzMeta.className}"><ha-icon icon="mdi:axis-arrow"></ha-icon>PTZ ${snapshot.ptz_supported ? "Ready" : "Off"}</span>
        <span class="${overlayMeta.className}"><ha-icon icon="mdi:video-wireless-outline"></ha-icon>Overlay ${this.escapeHtml(String(snapshot.webrtc_overlay_active))}</span>
        <span class="${streamMeta.className}"><ha-icon icon="mdi:play-network-outline"></ha-icon>${this.escapeHtml(snapshot.stream_mode || "auto")}</span>
        <span class="${speedMeta.className}"><ha-icon icon="mdi:speedometer"></ha-icon>Speed ${this.escapeHtml(String(snapshot.speed || "-"))}</span>
        <span class="${ptzDurationMeta.className}"><ha-icon icon="mdi:timer-outline"></ha-icon>PTZ ${this.escapeHtml(String(snapshot.ptz_duration || "-"))}ms</span>
        <span class="${lensDurationMeta.className}"><ha-icon icon="mdi:camera-control"></ha-icon>Lens ${this.escapeHtml(String(snapshot.lens_duration || "-"))}ms</span>
        <span class="${playbackMeta.className}"><ha-icon icon="mdi:play-box-multiple-outline"></ha-icon>Playback ${snapshot.playback_active ? "Live" : "Idle"}</span>
        <span class="${speakerMeta.className}"><ha-icon icon="mdi:volume-high"></ha-icon>Speaker ${snapshot.speaker_enabled ? "On" : "Off"}</span>
        <span class="${talkMeta.className}"><ha-icon icon="mdi:microphone"></ha-icon>Talk ${snapshot.talk_active ? "Live" : "Idle"}</span>
        <span class="${activityMeta.className}"><ha-icon icon="mdi:pulse"></ha-icon>Activity ${activityMeta.label}</span>
      </div>`;
  }
  ...
  getCardSize() {
    if (this._isMiniCardMode()) return 6;
  ...
  findEntityForChannel(channel, kind) {
    ...
    const exactMatchers = {
      camera: (entityId) => entityId.startsWith("camera.") && entityId.endsWith(`_camera_${wanted}`),
      info: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_info`),
      stream: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_stream`),
      online: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_online`),
      ptz_supported: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_ptz_supported`),
      motion: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_motion_alarm`),
      video_loss: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_video_loss_alarm`),
      intrusion: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_intrusion_alarm`),
      line_crossing: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_line_crossing_alarm`),
      tamper: (entityId) => entityId.startsWith("binary_sensor.") && entityId.endsWith(`_camera_${wanted}_tamper_alarm`),
      audio_enabled: (entityId) => entityId.startsWith("binary_sensor.") && (entityId.endsWith(`_camera_${wanted}_audio_enabled`) || entityId.endsWith(`_camera_${wanted}_enabled`)),
      audio_classifier_enabled: (entityId) => entityId.startsWith("binary_sensor.") && (entityId.endsWith(`_camera_${wanted}_audio_classifier_enabled`) || entityId.endsWith(`_camera_${wanted}_classifier_enabled`)),
      audio_gunshot: (entityId) => entityId.startsWith("binary_sensor.") && (entityId.endsWith(`_camera_${wanted}_audio_gunshot_detected`) || entityId.endsWith(`_camera_${wanted}_gunshot_detected`)),
      audio_level: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_audio_level`),
      audio_peak: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_audio_peak`),
      audio_classifier_label: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_classifier_label`),
      audio_classifier_confidence: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_classifier_confidence`),
      audio_classifier_threshold: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_classifier_threshold`),
      audio_last_event: (entityId) => entityId.startsWith("sensor.") && entityId.endsWith(`_camera_${wanted}_audio_last_event`),
      audio_stream_status: (entityId) => entityId.startsWith("sensor.") && (entityId.endsWith(`_camera_${wanted}_audio_stream_status`) || entityId.endsWith(`_camera_${wanted}_native_stream_status`)),
...
_renderAudioConsoleOverlay(refs = {}, streamMode = "", playbackActive = false) {
  ...
  render() {
    ...
    const cardMode = this._normalizeCardMode(this.config.card_mode);
    ...
    const showCapabilityBanner = !isMiniMode;
    const showAccessoryOverlays = !isMiniMode;
...
.hik-health-chip { display:inline-flex; align-items:center; gap:6px; border-radius:999px; padding:4px 8px; font-size:11px; font-weight:700; letter-spacing:0.04em; }
          ...
          .hik-select { min-height:34px; border:none; border-radius:12px; padding:0 10px; background: color-mix(in srgb, var(--secondary-background-color) 72%, #0b1220); color: var(--primary-text-color); -webkit-text-fill-color: currentColor; }
          ...
          .hik-video-media-btn.is-enabled { background:rgba(16,32,24,0.52); border-color:rgba(92,214,140,0.34); box-shadow:0 0 0 1px rgba(92,214,140,0.16), 0 12px 26px rgba(0,0,0,0.28); }
          .hik-video-media-btn.is-talking { background:rgba(52,20,24,0.56); border-color:rgba(255,110,110,0.34); box-shadow:0 0 0 1px rgba(255,110,110,0.16), 0 12px 26px rgba(0,0,0,0.28); }
          ...
          .hik-video-mini-select select, .hik-video-mini-input { background:rgba(8,12,18,0.92); color:var(--primary-text-color); border:none; outline:none; font-size:12px; min-width:112px; border-radius:10px; -webkit-text-fill-color:currentColor; }          .hik-video-mini-select select:focus, .hik-video-mini-input:focus, .hik-select:focus { background:rgba(10,14,20,0.98); box-shadow:0 0 0 1px rgba(93, 173, 255, 0.26); }          .hik-video-mini-select select option, .hik-video-mini-select select optgroup, .hik-select option, .hik-select optgroup { color:#f5f7fa; background-color:#111827; }
          ...
          .hik-debug-resize-handle { position:absolute; right:0; bottom:0; width:46px; height:46px; border:none; background:transparent; cursor:nwse-resize; pointer-events:auto; touch-action:none; z-index:2; }
          .hik-debug-resize-handle::before { content:""; position:absolute; right:10px; bottom:10px; width:18px; height:18px; border-right:2px solid rgba(135,247,185,0.68); border-bottom:2px solid rgba(135,247,185,0.68); opacity:0.9; }
...
@keyframes hikAudioConsolePulse {
  ...
  50% { box-shadow: 0 0 0 6px color-mix(in srgb, var(--hik-accent) 14%, transparent); }
          ...
          .hik-debug-snapshot-pill { position:relative; overflow:hidden; transition:transform 180ms ease, box-shadow 180ms ease, filter 180ms ease, border-color 180ms ease; }
          .hik-debug-snapshot-pill.is-live { transform:translateY(-1px); box-shadow:0 0 0 1px rgba(255,255,255,0.06), 0 0 0 3px color-mix(in srgb, var(--hik-accent) 18%, transparent), 0 10px 22px rgba(0,0,0,0.18); filter:saturate(1.18) brightness(1.04); animation:hik-debug-pill-pulse 1.1s ease-out 0s 2; }
          .hik-debug-snapshot-pill.is-live::after { content:""; position:absolute; inset:0; background:linear-gradient(120deg, transparent 0%, rgba(255,255,255,0.18) 48%, transparent 100%); transform:translateX(-120%); animation:hik-debug-pill-sheen 1.1s ease-out 0s 2; pointer-events:none; }
          @keyframes hik-debug-pill-pulse { 0% { transform:translateY(0) scale(1); box-shadow:0 0 0 0 color-mix(in srgb, var(--hik-accent) 0%, transparent); } 35% { transform:translateY(-1px) scale(1.012); box-shadow:0 0 0 4px color-mix(in srgb, var(--hik-accent) 22%, transparent); } 100% { transform:translateY(-1px) scale(1); box-shadow:0 0 0 0 color-mix(in srgb, var(--hik-accent) 0%, transparent); } }
          @keyframes hik-debug-pill-sheen { 0% { transform:translateX(-120%); opacity:0; } 25% { opacity:1; } 100% { transform:translateX(120%); opacity:0; } }
          ...
          .hik-debug-dashboard-body { display:grid; gap:0; }
        ...
        <div class="hik-wrap ${isMiniMode ? "is-mini" : ""}">
          ${showTitleBar ? `
            ...
            <div class="hik-panel hik-video-shell">
              <div class="hik-merged-shell">
                <div class="hik-video-block ${playbackActive ? "is-playback" : "is-live"} ${isMiniMode ? "is-mini" : ""}">
                  ...
                  ` : ""}
                  ${isMiniMode ? "" : this.renderPlaybackOverlay(playbackIndicator)}
                  <div class="hik-video-media-overlay">
                    ${showStatusPillsOverlay ? this._renderStatusPillsOverlay(versionInfo, {
                    ...
                    ${showAccessoryOverlays ? audioConsoleOverlayContent : ""}
                    ${showAccessoryOverlays ? streamModeOverlayContent : ""}
                    ${showAccessoryOverlays ? streamInfoOverlayContent : ""}
                    ${showAccessoryOverlays ? cameraInfoOverlayContent : ""}
                    ${showBottomSelectors && (!this._gridMode && !playbackActive && !this._playbackOverlayVisible) ? `
                    ...
                    ${showPlaybackPanel ? `
                      <div class="hik-video-playback-panel ${playbackActive ? "is-recording" : "is-standby"}">
                        ...
                        <div class="hik-video-playback-actions">
                          ...
                          <button type="button" class="hik-video-media-btn" id="hik-playback-start-overlay" title="Start playback"><ha-icon icon="mdi:play"></ha-icon></button>
          ...
          ${showInfoGrid ? `<div class="hik-info-grid">${infoCards.join("")}</div>` : ""}
        </div>
      </ha-card>
    `;
      ...
      <div style="padding:16px;display:grid;gap:16px;">
        ...
        ${this._section(
          ...
          `
            ...
            ${this._field("card_mode", "Card mode", `
              <select id="card_mode" style="width:100%;">
